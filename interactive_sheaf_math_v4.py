...
def get_matrix_input(rows, cols, name):
    ...
    print(f"\n--- {name} ({rows}x{cols}) ---")
    print("Shortcuts: 'id' (Identity), '-id' (Negative Identity), '0' (Zero)")
    
    shortcut = input("Entry (or hit Enter for manual): ").strip().lower()
    
    if shortcut == 'id':
        return np.eye(rows, cols)
    elif shortcut == '-id':
        return -1.0 * np.eye(rows, cols)
    elif shortcut == '0':
        return np.zeros((rows, cols))
    
    # Manual entry
    print(f"Manual Entry: Enter {rows} rows of {cols} numbers each.")
    matrix = []
    for i in range(rows):
        while True:
            try:
                ...
                if len(row_input) != cols:
                    print(f"Error: Expected {cols} numbers, got {len(row_input)}.")
                    continue
