...
def save_test_results(filename, label, mode, n_v, edges, v_labels, e_labels, energy, num_sections, null_space, L, spectrum):
    if not filename.endswith(".txt"): filename += ".txt"
    with open(filename, "a", encoding="utf-8") as f:
        f.write(f"\n{'='*70}\n")
        f.write(f"LABEL: {label} | MODE: {mode.upper()}\n")
        ...
        for i, e_s in enumerate(e_labels):
            ...
            f.write(f"Edge {i} {edges[i]}: {{{e_str}}} (Dim {len(e_s)})\n")

        f.write("\n--- CALCULATED RESULTS ---\n")
        if L.size == 0:
            f.write("TOTAL SHEAF DIMENSION IS 0.\n")
        else:
            f.write(f"SPECTRAL ENERGY: {round(energy, 6)}\n")
            f.write(f"H0 DIMENSION: {num_sections}\n")
...
while True:
    print("\n" + "="*45 + "\nSHEAF CALCULATOR: INTERSECT vs SYMM-DIFF\n" + "="*45)
    
    # Mode Selection
    print("Choose Edge Label Mode:")
    print("1: Intersection (v_u ∩ v_v)")
    print("2: Symmetric Difference (v_u Δ v_v)")
    mode_choice = input("Choice (1 or 2): ").strip()
    mode_name = "intersection" if mode_choice == '1' else "symmetric_difference"
    ...
    try:
        ...
        print(f"Energy: {round(energy, 4)} | H0 Dim: {null_space.shape[1]}")
        
        save_test_results(output_file, test_label, mode_name, n_v, edges, v_labels, e_labels, 
